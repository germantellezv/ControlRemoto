...
class Main2Activity : AppCompatActivity(), View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        detener!!.setOnClickListener{
            var url="http://10.3.141.1:5000/api/v1/stop?token=$token"
        ...
        desconectar!!.setOnClickListener{
            var url="http://10.3.141.1:5000/api/v1/logout"
                .httpGet().responseJson{ request, response, result ->
                    when (result) {
                        is Result.Failure -> {println("hubo un fallo")
                        }
    ...
    override fun onClick(p0: View?) {
        ...
        var baseUrl:String = "http://10.3.141.1:5000/api/v1/move"

        //var url="htpp://10.3.141.1:5000/api/v1/move/$motor/$direccion/$velocidad/$tiempo"+"?token=$token"
